...
void test_config_all(void)
{
    ...
    && (MANET_FRAME_DURATION_US == 160000L) \
    && (MANET_VOICE_PAYLOAD_BITS == 512L) && (MANET_ADDR_BITS == 8u) && (MANET_SEQ_BITS == 8u) && (MANET_TTL_BITS == 4u)
    /* Straw-man values below. Skipped when the configuration is swept — see
     * tools/budget.sh and `make test-3slot`. */
    /* Frame structure */
    CHECK_EQ(MANET_SLOT_DURATION_US, 40000);
    CHECK_EQ(MANET_GUARD_US, 3320);
    CHECK_EQ(MANET_BURST_US, 36680);
    ...
    CHECK_EQ(MANET_SLOT_RAW_BITS, 768);
    CHECK_EQ(MANET_SLOT_ONAIR_BITS, 704);
    CHECK_EQ(MANET_FEC_BITS_AVAILABLE, 94);
    CHECK_EQ(MANET_FEC_PERCENT, 16);
